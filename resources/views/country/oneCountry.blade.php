    ...
    <div class="container">
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-heading"><h2>{{ $oneCountry->name }}</h2></div>
                        ...
                        @if (count($foodList) > 0)
		                    ...
		                    <form action="{{ url('#') }}" method="POST" class="form-horizontal">
		                        ...
		                        <div class="form-group" >
		                         ...
		                         <input id="filterOption" type="hidden" name="filterOption" value="newestToOldest">
		                    </form>
	                        ...
	                        <table class="table table-striped task-table"  id="myTable">
	                            ...
	                            @foreach($foodList as $food)
	                                <tr>
	                                    <!-- image, show zero or one -->
	                                    <td class="table-text" width="70%">
	                                    	@if ($food->images->isNotEmpty())
	                                        	<img src="{{Storage::disk('public')->url($food->images->first()->filename)}}" alt="{{$food->images->first()->filename}}">
	                                        @else
	                                        	<img src="{{URL::asset('/img/not_available.jpg')}}" alt="img not available">
	                                        @endif
	                                    </td>
